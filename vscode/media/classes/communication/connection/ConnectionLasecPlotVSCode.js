...
class ConnectionLasecPlotVSCode extends Connection {
    ...
    sendUdpText(input, text) {
        if (!text || !text.length) return;
        const msg = { data: text };         // extension.ts captura a chave 'data'
        try {
    ...
    connect() {
        ...
        window.addEventListener('message', message => {
            ...
            if (msg && msg.kind === 'udp-ready') {
                // garanta uma conexão VSCode com um input UDP "conectado"
                if (!app.connections.some(c => c.type === 'teleplot-vscode')) {
                    const conn = new ConnectionTeleplotVSCode('teleplot-vscode');
                    conn.supportSerial = true;
                    const udpInput = {
                        type: 'UDP',
                        name: 'UDP',
                        connected: true,
                        address: 'peer',      // será o lastUdpPeerIp no envio
                        port: msg.port,
                        cmdPort: msg.cmdPort,
                        textToSend: ''
                    };
                    conn.inputs.push(udpInput);
